...
import { 
  ...
  HelpCircle, Trophy, BookOpen, Clock, Activity, ArrowRight, Zap, Crown, Sparkles 
} from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      {!showDirections ? (
        <div className="space-y-12">
          ...
          <div className="bg-gradient-to-r from-amber-500 via-amber-600 to-orange-600 rounded-3xl p-6 sm:p-8 text-white shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden">
            <div className="space-y-2 z-10 text-center sm:text-left">
              ...
              <h3 className="font-display font-black text-2xl sm:text-3xl tracking-tight">
                Cheksiz Testlar va Premium Imtiyozlar!
              </h3>
              <p className="text-amber-100 text-xs sm:text-sm font-semibold max-w-xl">
                DTM imtihonlariga eng yuqori darajada tayyorlaning: barcha yo'nalishdagi testlar, batafsil tahlillar va promokodlar.
              </p>
            </div>
            <button
              onClick={() => setCurrentTab("premium")}
              className="z-10 px-6 py-3.5 bg-white hover:bg-amber-50 text-amber-800 font-extrabold text-sm rounded-2xl shadow-lg transition-all transform hover:scale-[1.02] flex items-center space-x-2 cursor-pointer flex-shrink-0"
            >
              <Sparkles className="w-4 h-4 text-amber-600" />
              <span>PREMIUM O'TISH / XARID QILISH</span>
            </button>
          </div>
