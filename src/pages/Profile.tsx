...
import { auth, rtdb, ref, get, update, remove } from "../lib/firebase";
import { deleteUser, signOut } from "firebase/auth";
import { User } from "../types";
import { getAccessRemainingText, getAccessTimeBreakdown } from "../lib/premium";
...
export default function Profile({ currentUser, viewedUser, onBack, onDeleteAccount, onUserUpdate }: ProfileProps) {
  ...
  useEffect(() => {
    const fetchExamHistory = async () => {
      setLoading(true);
      try {
        const snapshot = await get(ref(rtdb, "results"));
        const list: any[] = [];
        let totalSum = 0;
        let maxScore = 0;
        let passedTests = 0;
        ...
        if (snapshot.exists()) {
          const rawResults = snapshot.val();
          const allResults = Object.entries(rawResults || {}).map(([key, value]) => ({
            id: key,
            ...(value as any)
          }));

          const filtered = allResults
            .filter((item: any) => item.uid === displayUser.uid)
            .sort((a: any, b: any) => {
  ...
  const handleConfirmDeleteAccount = async () => {
    ...
    try {
      try {
        await remove(ref(rtdb, `users/${currentUser.uid}`));
      } catch (err) {
        console.warn("RTDB delete user error:", err);
      }

      try {
        await remove(ref(rtdb, `leaderboard/${currentUser.uid}`));
      } catch (err) {
        console.warn("RTDB delete leaderboard error:", err);
      }
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await update(ref(rtdb, `users/${currentUser.uid}`), fieldUpdates);
